...
export function renderGroup(group) {
  ...
  return (
    <span key={group}>
      {groups.map((group) => (
        <Tag size="large" color={tagColors[group] || stringToColor(group)} key={group}>
          {group}
        </Tag>
      ))}
    </span>
    ...
    return (
      <>
        <article>
          ...
          <p></p>
          <p>
            提示 {inputTokens} tokens / 1M tokens * ${inputRatioPrice} + 补全 {completionTokens}{' '}
            tokens / 1M tokens * ${completionRatioPrice} = ${price.toFixed(6)}
          </p>
        </article>
      </>
    );
